...
-- Test: calling `SetVisibility("GONE")`/`SetVisibility("VISIBLE")` on the
-- root container itself, after the fact, takes its frame down and back up,
-- same as giving `visibility = "GONE"` up front.
do
  ...
  assert(root._test.width == 200) -- the new width waits, no layout runs while `"GONE"`

  container:SetVisibility("VISIBLE")
  container:Layout()

  assert(root._test.showCalls == 2)
  assert(root._test.width == 300 and child._test.width == 300)
end
