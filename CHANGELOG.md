...
- `minWidth`/`maxWidth`/`minHeight`/`maxHeight` and their setters (any node): a floor or ceiling on a flexible child's own share of leftover main-axis space. Only the pair matching a node's own main axis does anything, `minWidth`/`maxWidth` in a ROW parent, `minHeight`/`maxHeight` in a COLUMN one. No effect on a node with its own explicit `width`/`height`, or `"AUTO"`. Errors if a node's own min is greater than its max. If clamping leaves every flexible child on a line without any more space to claim, `justify` distributes whatever's left, the same as when nothing on that line is flexible at all.
